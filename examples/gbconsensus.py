...
def enumerate_efms():
	...
	configuration[kp.K_SHORTEST_OPROPERTY_MAXSOLUTIONS] = 10

	drains = set([s for s in singles if "R_EX" in singles])
	...
	medium = consumed | produced

	irreversible_system = IrreversibleLinearSystem(S, irrev, drains, medium, consumed, produced)
	algorithm = KShortestEFMAlgorithm(configuration)

	return algorithm.enumerate(irreversible_system)
